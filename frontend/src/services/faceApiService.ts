...
class FaceApiService {
  ...
  private MODEL_PATH = '/models'; // Path where face-api models are stored
  private enrolledDescriptors: Map<string, Float32Array> = new Map();
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const canLoadFrom = async (base: string): Promise<boolean> => {
        try {
  ...
  /**
   * Detect the largest face in frame (by box area). Use this for check-in when multiple faces may appear (e.g. reflection, background).
   */
  private async detectLargestFaceFromVideo(videoElement: HTMLVideoElement): Promise<FaceDetectionResult> {
    if (!this.modelsLoaded) {
      throw new Error('Face-API not initialized');
    }
    try {
   ...
   */
  async detectMultipleFaces(videoElement: HTMLVideoElement): Promise<number> {
    if (!this.modelsLoaded) {
      throw new Error('Face-API not initialized');
    }
   ...
   */
  async enrollFace(videoElement: HTMLVideoElement, studentId: string): Promise<BiometricData> {
    try {
      // Capture initial face (largest in frame)
      ...
      // Wait for slight movement (basic liveness check)
      ...
      // Capture final face (largest in frame)
   ...
   */
  async verifyFace(videoElement: HTMLVideoElement, storedDescriptor: Float32Array): Promise<BiometricData> {
    try {
      // Use largest face in frame (avoids failing when multiple faces detected, e.g. reflection or poster)
      ...
      // Only compare with stored descriptor when one is provided (e.g. single-user device).
      // For kiosk check-in we have no stored descriptor; backend will match by descriptor similarity.
      let confidence = 0.95;
   ...
   */
  private async performLivenessCheck(videoElement: HTMLVideoElement, initialDescriptor: Float32Array): Promise<void> {
   ...
   */
  getStatus(): { modelsLoaded: boolean; enrolledCount: number } {
    return {
